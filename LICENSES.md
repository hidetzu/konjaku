...
⚠ **この表は `test/check.mjs` が `public/data/` の実体と突き合わせる。**
...
| `data/monument/` | 自然災害伝承碑（z8 のタイル） | **国土地理院**（`disaster_lore_all`） | 下記。⚠ **災害名・種別・碑文は出典の字のまま**。⚠ **碑があることと被災は別**（画面がそう言う） |
| `data/muni.json` | 市区町村コードから名前を引く表 | **国土地理院**（地理院地図の `muni.js`） | 下記 |
| `data/landuse-code.json` | 土地利用種別のコード表 | **国土交通省**（国土数値情報 土地利用細分メッシュ L03-b） | ⚠ **コード表をそのまま写したもの**。⚠ 商用利用可 |
| `data/area-record.json` | その地域について公式資料に書かれている記録 | **東京都港湾局「東京港の歴史」** ／ **浦安市「浦安市の海面埋め立て」** | ⚠ **原典の字をそのまま写したもの**。⚠ **要約しない。**⚠ 範囲の矩形はこちらが引いたもので、原典の一部ではない |
...
⚠ **この規則は `test/check.mjs` が見張る。**⚠ 上の表の「条件」に
`非商用` のような語が入ったら落ちる。
