...
((g) => {
  ...
  function createEraControl({ root, onChangeEra, onTogglePlay }) {
    ...
    function buildRuler(){
      ...
      // ⚠ **段の有無で分岐しない**（2026-08-23。hidetzu/konjaku#170）。
      //   ⚠ **明治期のデータが無い土地では、⚠ 段そのものを出さなくなった**（ADR 0026:
      //     ⚠ 押しても何も起きない段を置かない）。
      //   ⚠ **段で分岐すると、⚠ そのとき断りごと消える**（⚠ 実測 2026-08-23・札幌:
      //     ⚠ 「空中写真 5 段 ／ 明治期はこの土地では未整備」→「空中写真 5 段」）。
      //   ⚠ **消すなら、⚠ どこで読めるかを同時に言う**（掟 §1）。
      //   ⚠ **スマホの初期画面では、⚠ ここが唯一その事実に触れる場所**
      //     （⚠ パネルの「整備対象外」は、⚠ 小さいあいだ畳まれている）。
      const meijiHas = meijiHasNow;
      // ⚠ **明治期だけ年が無いことを、⚠ ここで言う**（2026-08-28。hidetzu/konjaku#142）。
      //   ⚠ **利用者役 4/4 が「年が無いのは、⚠ 年代を切りかえるときに知りたい」と答えた**
      //     （⚠ 実在の利用者ではない。`CLAUDE.md` §4-1）。
      //   ⚠ **2/4 は「目盛りの右端が明治期だけ違う印なのが、⚠ 説明なしでは分からなかった」。**
      //   ⚠ **年を出すのではない。**⚠ **持っていない、と言う**（⚠ 低湿地データは年を持たない）。
      rlNote.textContent=photo.length
        ? `空中写真 ${photo.length} 段`
          +(meijiHas ? " ／ 明治期は地図（年は無い）" : " ／ 明治期はこの土地では未整備")
        : "";
    }
