...
// ⚠ **地理院タイルの入口は、verify.js が持つ 1 つだけ。**
//   ⚠ 2 か所に書くと、片方だけ直したときに**同じ画面で別の相手を見る**
//   （掟: 同じ問いに答える実装を2つ持たない）。
//   ⚠ verify.js は peel.html の中でこれより先に読まれている（読み込み順が前提）。
...
const WORD = {
  ...
  //   ⚠ 「判定できません」の字は words.js（トップの根拠カードと同じ語）
  cantSay: (hasLandform) => hasLandform ? "建物ごとには出せません" : KonjakuWords.S.cantTell,
  // ⚠ **層の名前は、その層が答えている「問い」で呼ぶ**（docs/DOMAIN.md §1）。
  //   ⚠ 「第1層」「第2層」は内部の呼び名。**画面に出さない**。
  // ⚠ **字は words.js の 1 か所**（2026-08-20 に移した。hidetzu/konjaku#122）。
  //   ⚠ トップにも同じ見出しを出すので、⚠ **持ち主が 2 か所になるのを避けた。**
  layerTitle: (n) => KonjakuWords.layerTitle(n),
  // ⚠ 第1層。**主語は「この土地は」に統一**（実測 2026-08-19: 利用者役 2/4 が
  //   「この場所」と「この土地」を別物と読んだ）。
  //   ⚠ **時間の語（もとは・昔は）を使わない。**3/4 が明治期（第2層）と取り違えた。
  //   ⚠ 成因と人の手は**行を分ける**。1 行に混ぜると「もとから埋立地だった」に読める。
  // ⚠ **字は words.js の 1 か所**（2026-08-20 に移した）。⚠ **太字にするかは見た目なのでここが決め、
  //   ⚠ 語そのものは words.js が決める。**⚠ 共有カードの文は素のまま同じ語を出す。
  ground1: (name) => KonjakuWords.ground1Lines(name, null, (v) => `<b>${v}</b>`)[0],
  ground1Art: (art) => KonjakuWords.ground1Lines("", art, (v) => `<b>${v}</b>`)[1],
...
// ⚠ 実測（2026-08-18・豊洲）。何が拾えるかは落とし方で違う:
...
//
// ⚠ 混ぜていたときの実測（2026-08-18・豊洲・1280×900）:
...
//   実測（2026-08-18）: 403 と通信断は map.on("error") で
