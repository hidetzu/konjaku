...
((g) => {
  ...
  // ⚠ **整備対象外のときは、⚠ ここでは何も言わない**（2026-08-23。Owner 判断）。
  //   ⚠ **層の理由が既に「この範囲は明治期の低湿地データの整備対象外です」と言っている。**
  //   ⚠ **同じことを 2 行言っていて、⚠ 2 行目が「無い」と言い切っていた**（掟 §1:
  //     ⚠ **データにない ≠ 現実にない**）。⚠ **消したのは繰り返しであって、⚠ 事実ではない。**
  //   ⚠ **読み込めなかったときは残す**（⚠ こちらは層の理由と別のことを言っている）。
  const waterRow = (area) => (!area || area.waterRead)
    ? { q: 2, level: OK, tag: "実測", body: "水面の形は低湿地データから起こした<b>実際の水域</b>" }
    : area.waterUnread
      ? { q: 2, level: NO, tag: "未取得", body: "明治期の低湿地データを読み込めていない" }
      : null;
  ...
  // ⚠ **`null` を返す行がある**（2026-08-23。⚠ 整備対象外のときの `waterRow`）。
  //   ⚠ **層の理由が既に言っていることを、⚠ ここで繰り返さない**ため。
  const rows = (s) => [
    ...landRow(s),
