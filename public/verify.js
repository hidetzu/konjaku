...
(function (global) {
  ...
  // 地形分類の区分を、平易な言い方に**添える**（置き換えではない）。
  // ⚠ **区分名は消さない。** 国土地理院の区分名で、成因と災害リスクの記述がこれに紐づく。
  //   消すと、いま出している「液状化の発生傾向が非常に強い」の根拠まで消える。
  // ⚠ **ここで書いているのは「いつの話か」だけ**で、区分の意味は国土地理院の記述
  //   （`landform.json` の why）が持つ。意味をこちらで言い換え始めると原典と食い違う。
  //   ⚠ 定義に「かつて」とある区分だけを対象にする。時期が特定できないことを、そう書く。
  const PLAIN_PAST = {
    "旧水部": "水だった土地",   // ⚠ 人工地形が無いときは「水だった土地（旧水部）です。」
    "旧河道": "川が流れていた土地",
    "干拓地": "水面か低湿地だった土地",
    "落堀":   "洪水で掘られた跡",
  };
  // ⚠ **「もとは」と言えるのは、人の手が入る前の姿を指しているときだけ。**
  //   1) 定義に「かつて」がある区分（旧水部・旧河道・干拓地・落堀）… それ自体が過去を指す
  //   2) 人工地形（盛土地･埋立地など）が乗っている土地 … 自然の地形は**その下＝手が入る前**の姿
  //      根拠: 盛土地･埋立地の定義は「周囲の地表より高く盛土した土地や、水部に土砂を投入して
  //      陸地にしたり…」で、**あとから人が手を入れた**と書かれている。
  //   ⚠ 人工地形が無い土地では「もとは」と言わない。`氾濫平野・海岸平野` は
  //     「起伏が小さく、低くて平坦な土地」＝**いまもその地形**で、過去形にすると原典より強くなる。
  //     （実際、氾濫平野で「この場所は…で、いまは…」のまま残っていたのをオーナーが見つけた）
  function plainPast(value, artificial) {
    const p = PLAIN_PAST[value];
    if (!artificial) {
      // 人の手が入っていない土地。⚠ いまの姿として言う（過去形にしない）
      return p ? `${p}（${value}）です。` : `この場所は ${value} です。`;
    }
    // 人の手が入っている土地。自然の地形は「もとは」＝手が入る前の姿
    // ⚠ 言い換えがあるときは「…（旧水部）」、無いときは「氾濫平野・海岸平野 」。
    //   どちらも直後に「です。」が続く形に揃える
    const was = p ? `${p}（${value}）` : `${value} `;
    // ⚠ 空白を入れない。`${was}` は「…（旧水部）」で終わるので、
    //   「（旧水部） です。」と余分な空きが出ていた（実測で見つけた）
    return `もとは ${was}です。いまは ${artificial} です。`;
  }
  ...
  function narrate(f) {
    ...
    if (l.ok) {
      // ⚠ **「もとは」を先に、「いま」を後に。** 看板は「この土地は、昔なんだったのか？」なので、
      //   「いまは〜」から始めると質問と答えがずれる。初見の 3 人がそう答えた（2026-08-17）:
      //   「質問と答えがズレていて気持ちが悪い」「案2 だけ見せられたら**不足に気づけない**」
      //   「『いまは盛土地･埋立地』だけだと**悪い意味に取れない**。『もとは水だった』で意味が反転した」
      // ⚠ **専門用語を主役にしない。** 効いていたのは区分名ではなく**平易な言い換え**だった。
      //   3 人が「旧水部は無くても困らない」「括弧の中身だけ残して外を消してほしい」と答えた。
      //   ただし区分名は残す（国土地理院の区分名で、災害リスクの記述がこれに紐づいている）。
      // ⚠ **時期を書かない、と分かるように書く。** 旧水部の定義は
      //   「旧版地形図や古い空中写真等の資料から、**かつて**…水部であったと確認できた土地」で、
      //   **明治期に限定していない**。下に出る「明治期は〜」と並ぶと同じ時期の話に読まれ、
      //   実際に初見の人が「泥地＋水で約50%、半分は水気があった」と**誤った統合**をした。
      // ⚠ **人工地形が無い土地でも文が完成すること。** 最初 `plainPast()` が
      //   「この場所は 扇状地 で、」と**途中で切れる**形を返していて、札幌・軽井沢で
      //   文が尻切れになった（2026-08-17 に実測で気づいた）。
      //   → 言い換えの有無と、人工地形の有無を**別々に**組む。
      out.push(plainPast(l.value, l.artificial));
