...
(function (global) {
  ...
  function plainPast(value, artificial) {
    ...
    const now = artificial ? `いまは ${artificial} です。` : "";
    // ⚠ 表に無い区分は、これまでどおり区分名のまま言う。勝手な言い換えを増やさない。
    //   ⚠ そのときも、人工地形の有無で文が完成すること（尻切れにしない）。
    if (!p) return artificial
      ? `この場所は ${value} で、いまは ${artificial} です。`
      : `この場所は ${value} です。`;
    return `もとは ${p.replace(/^もとは /, "")}（${value}）です。${now}`;
  }
