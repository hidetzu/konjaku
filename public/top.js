...
(() => {
  ...
  async function drawFace(left, top, w, h) {
    ...
    // ⚠ **詳細版が 1 面も取れなかったら、⚠ 広域版で塗る**（2026-09-06。Owner 判断）。
    //   ⚠ **判定のほうは前から広域版へ落ちていた。**⚠ **地図だけが落ちていなかった。**
    //   ⚠ **実測（2026-09-06・本番）**: ⚠ **軽井沢・白馬は詳細版が 404 で、⚠ 広域版に面が在る。**
    //     ⚠ **答えの 2 行目は区分の名前を出すのに、⚠ 地図は 0 画素だった。**
    //   ⚠ **利用者役 3 名のうち 1 名が、⚠ 文章のほうを疑った**（⚠ 実在の利用者ではない）。
    //   ⚠ **粗いことは、⚠ 画面が別に言う。**⚠ **ここで決めるのは、⚠ 塗るかどうかだけ。**
    if (!tally.size) {
  ...
  function drawLegend() {
    ...
    // ⚠ **いちばん狭い幅では 2 種まで**（2026-09-06。Owner 判断。⚠ 絵を並べて決めた）。
    //   ⚠ **3 種だと凡例が 2 段になり、⚠ 板が画面の 47%・見える地図が 177px まで落ちる。**
    //   ⚠ **2 種なら 1 段で、⚠ 44% / 201px**（⚠ 実測 320×640・軽井沢）。
    //   ⚠ **3 つ目は消えない。**⚠ **「ほか n 種を見る」に回る。**
    //   ⚠ **「ここ」の区分は必ず 1 つ目に入るので、⚠ 自分の足元は必ず見える。**
    //   ⚠ **幅の数はここに書かない。**⚠ **CSS の 1 か所（`#legend::before`）が決め、⚠ ここは読むだけ**
    //     （⚠ `.fold::before` と同じやり方。`CLAUDE.md` §3）。
    const 上限 = Number(getComputedStyle(legendEl, "::before").content.replace(/"/g, "")) || 3;
    for (const nm of 順) {
      if (出す.length >= 上限) break;
  ...
  function drawShare() {
    ...
    const q = KonjakuPlaceArg.placeQuery({ lat: px2lat(cy), lon: px2lon(cx) });
    deepLink.href = "./deep" + (q ? q.replace(/^\?q=&/, "?") : "");
  }
