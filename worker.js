...
const EVENTS = new Set([
  ...
  "judged.fail",   // 読み込めなかった（通信断・タイムアウト・403 などの拒否）
  "shared",        // 共有した（分子）
  "saved",         // 画像として保存した
  // ⚠ **共有の 3 つの結末**（2026-08-28・hidetzu/konjaku#355）。
  //   ⚠ **`shared` と `saved` だけだと、⚠ 0 件が何を意味するか分からなかった。**
  //   ⚠ **押していない ／ 押したがやめた ／ 押したが壊れた**が、⚠ **同じ 0 に見えていた**
  //   （⚠ D1 実測 2026-08-28: ⚠ 17 日間 `shared` 0 件・`saved` 0 件）。
  // ⚠ **`share.tap` が分母。**⚠ 結末は排他なので、⚠ 1 共有あたり最大 2 件。
  "share.tap",       // 共有を押した（⚠ 分母。⚠ ここが 0 なら「押されていない」）
  "share.cancelled", // 押した → やめた（共有シートを閉じた。⚠ 失敗ではない）
  "share.failed",    // 押した → 壊れた（⚠ 絵が作れない・保存に失敗した）
...
const TARGETS = new Set(["all", "landform", "meiji", "elevation", "photos",
  // ⚠ **2026-08-28 まで、⚠ どこからも送っていなかった**（hidetzu/konjaku#354）。
  //   ⚠ **列挙だけ在って、⚠ 送る側が無かった。**⚠ **入口（住所検索）が全滅しても記録がゼロ**で、
  //   ⚠ **D1 に 17 日間 1 行も無かった**（実測 2026-08-28）。
  //   ⚠ **いまは `public/share.js` の `searchHealth()` が送る。**
  //   ⚠ **列挙が片側にしか無い状態は、⚠ `test/check/data.mjs` が落とす。**
  "search",
...
export default {
  async fetch(req, env) {
    ...
    //
    // ⚠ 上限は 48 のまま変えない。受け付ける本文は **32 種**で、最長は
    //   `health:elevation:fail` の **21 文字**（2026-08-28 実測。下の EVENTS /
    //   TARGETS×{ok,fail} / SOURCES から数えたもので、test/check/safety.mjs が毎回数え直す）。
