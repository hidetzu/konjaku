...
//   ⚠ 「明治期は陸だった建物が 1364 件」と読める（データの話が、土地の話に化けている）。
{
  ...
  if (!m || !mw) bad("peel3d.js の breakdown を取り出せない（この検査が何も見ていない）");
    ...
    {
      const A = { total:100, dated:3, unread:20, hSrc:{measured:40,levels:10,default:50} };
      const h = paintTo({ "読み込めず": 20, "旧水部": 80 }, 100, A);
      // ⚠ **3 つとも、⚠ 同じ分母（総数）で書く**（掟 §6）
      yes((h.match(/ \/ 100</g) ?? []).length === 3,
        `内訳の分母が総数になっていない: ${(h.match(/ \/ \d+</g) ?? ["(無し)"]).join()}`);
      // ⚠ **色見本は、⚠ 押したとき地図で実際に変わる色の隣にだけ出す**
      //   （2026-08-23。Owner 判断。⚠ **主張は「照合できない色見本を出さない」**）。
      //   ⚠ **前は 3 行とも付いていたが、⚠ 2 つは地図に対応する相手がいなかった。**
      //     ⚠ `足元が分かる` の水色 = 「明治期に水だった」の色（⚠ 4832 件の色ではない）
      //     ⚠ `高さが実測` の砂色 = 「水でなかった」の色（⚠ 実測の建物は両方の色になる）
      //   ⚠ **見るのは数ではなく対応**（⚠ 数だけ見ると、⚠ 場所が違っても緑になる）。
      const legends = (h.match(/class="legend"/g) ?? []).length;
      const peeks = (h.match(/class="peek"/g) ?? []).length;
      yes(legends === peeks, `色見本 ${legends} 個 / 光らせるボタン ${peeks} 個（数が合っていない）`);
      yes(peeks > 0, "光らせるボタンが 1 つも無い（色見本の主張が空になる）");
      // ⚠ **色見本は、⚠ ボタンより後ろ**（⚠ 「押すと」の説明なので、⚠ 押す的の隣）
      yes(h.indexOf('class="peek"') < h.indexOf('class="legend"'),
        "色見本が、光らせるボタンより前に出ている（何の色か分からない）");
