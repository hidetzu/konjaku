...
//   ⚠ **いまは 4 通りある**（⚠ 4 つ目は 2026-09-06 に足した。hidetzu/konjaku#495。Owner 判断）:
//     明治期に区分がある      ここは 田 でした（⚠ 上のラベルが「明治期の地図」と名乗る）
//     地形分類が昔を名指す    ここは、かつて水面で、その後陸地にされた土地
//     明治期は無いが地形は在る ここは 低地（水の流れがつくった、平らな低い土地）
//                             ⚠ **無いことは、⚠ 答えの下の断りへ移した**（⚠ 消していない）
  ...
  CASES.push({
    ...
    async check(page) {
      ...
      const r = await page.evaluate(() => {
        ...
        const note = document.getElementById("glossNote");
        return {
          A: !!A,
          ...
          見出し: t("#gloss"),
          二行目, 区分,
          断り: note && !note.hidden ? t("#glossNote") : "",
          NONE: A ? A.MEIJI_NONE : {},
        };
      });
      ...
      // ⚠ **明治期が答えたかどうかで、⚠ 見るところが変わる。**
      //   ⚠ **明治期が答えなかったときは、⚠ 2026-09-06 から地形分類が見出しに来る**
      //     （Owner 判断。hidetzu/konjaku#495）。⚠ **無いことは、⚠ 答えの下の断りへ移した。**
      if (/^ここは .+ でした$/.test(r.見出し)) {
        // ⚠ **明治期が答えを返した土地。**
      ...
      } else if (Object.values(r.NONE).includes(r.見出し)) {
        // ⚠ **明治期も地形分類も無い土地。**⚠ **言えるのは、⚠ なぜ無いかだけ。**
        must(r.出典ラベル === r.SOURCE.meiji,
          `無い理由を言うのに、ラベルが違う: ${r.出典ラベル}`);
      } else {
        // ⚠ **明治期は無いが、⚠ 地形分類が読めている土地。**
        must(r.出典ラベル === r.SOURCE.terrain,
          `地形分類が見出しなのに、⚠ ラベルが違う: ${r.出典ラベル}`);
        // ⚠ **無いことを消していないこと**（掟 §1）。⚠ **断りが引き受ける。**
        must(r.断り, "明治期が無いことを、⚠ どこにも書いていない（⚠ 限界が消えた）");
        must(r.断り.includes(r.SOURCE.meiji),
          `断りが、⚠ 何の資料の話か名乗っていない: ${JSON.stringify(r.断り)}`);
      }
      return `${r.出典ラベル}｜「${r.見出し}」／ 2 行目「${r.二行目 || "（無し）"}」`
        + `／ 断り「${r.断り || "（無し）"}」／ 区分 ${r.区分}`;
    },
  });
}
...
//   ⚠ **明治期が答えた土地では、⚠ 成り立ちが 2 行目に出る**（⚠ 豊洲）。
//   ⚠ **明治期が答えなかった土地では、⚠ 2026-09-06 から地形分類が見出しへ上がり、
//     ⚠ 2 行目は空になる**（⚠ 軽井沢。Owner 判断。hidetzu/konjaku#495）。
//     ⚠ **空の行は隙間になるので、⚠ 行ごと隠す。**
//     ⚠ **無いことは消えていない。**⚠ **答えの下の断りが引き受ける**（⚠ そちらは別の検査が見る）。
...
for (const [名, ll, 詳細版] of [
  ...
  CASES.push({
    ...
    async check(page) {
      ...
      // ⚠ **答えに出ている区分が、⚠ 凡例にも在る**（⚠ 文章と地図が同じことを言う）。
      //   ⚠ **どちらの行に出るかは土地による**（2026-09-06。hidetzu/konjaku#495）:
      //     ⚠ 明治期が答えた土地   → 2 行目（⚠ 見出しは明治期の答え）
      //     ⚠ 明治期が無い土地     → 見出し（⚠ 2 行目は空）
      const 足元 = r.凡例[0].replace("（ここ）", "");
      must(`${r.見出し}${r.二行目}`.includes(足元),
        `答えと凡例が違う区分を言っている: 凡例「${足元}」`
        + `／ 見出し「${r.見出し}」／ 2 行目「${r.二行目}」`);
...
for (const [名, viewport] of [["PC", PCな幅], ["スマホ", SP], ["いちばん狭い幅", { width: 320, height: 640 }]]) {
  CASES.push({
    ...
    async check(page) {
      ...
      // ⚠ **単独の 0% だけを見る。**⚠ **「20%」「70%」の中の「0%」を拾わない**
      //   （⚠ 2026-09-02 に、⚠ 本番の豊洲 20% で自分の判定が誤爆した。
      //    ⚠ 実描画は軽井沢の値に固定していたので、⚠ ここでは出ていなかった）。
      must(!/(^|[^\d.])0(\.0+)?%/.test(r.全部の字),
        `⚠ 画面に 0% と書いている: ${(/(^|[^\d.])0(\.0+)?%/.exec(r.全部の字) ?? [])[0]}`);
      // ⚠ **「無い」とも書かない**
  ...
  for (const [名, viewport] of [["スマホ", SP], ["いちばん狭い幅", { width: 320, height: 640 }]]) {
    CASES.push({
      ...
      async check(page) {
        ...
        // ⚠ **「土地が … に変わります」はやめた**（2026-09-05。Owner 指示）。
        //   ⚠ **向こうが「水部」のことがある。**⚠ **水部は土地ではない。**
        must(/境目/.test(r.字), `何の話かを言っていない: 「${r.字}」`);
        for (const 狭い of ["土地が", "地面が"])
          must(!r.字.includes(狭い), `「${狭い}」と言っている（⚠ 向こうが水部のことがある）: 「${r.字}」`);
...
// ⚠ **自前の計測を、⚠ プライバシーポリシーが黙っていないか**（2026-09-08。`docs/adr/0102` / `0103`）。
//   ⚠ **前は要約の 1 行だけだった。**⚠ **本文は配信の仕組み（Cloudflare）の話しか持っていなかった。**
//   ⚠ **画面が「ここに書いていないことは、していません」と言っているので、⚠ 書かないと嘘になる。**
//   ⚠ **端末の中に印を置くことは、⚠ 読む人の判断が変わるところ**（⚠ 消す手立てが要るかどうか）。
CASES.push({
  name: "プライバシーポリシーが、⚠ 自前の計測と端末の印を言う",
  path: "/privacy", origin: NEXT_BASE, viewport: SP,
  async check(page) {
    const r = await page.evaluate(() => {
      const 節 = [...document.querySelectorAll(".about__sec")]
        .find((s) => /数えている/.test(s.querySelector(".about__h2")?.textContent ?? ""));
      if (!節) return null;
      return {
        文: 節.textContent.replace(/\s+/g, " ").split("。").map((x) => x.trim()).filter(Boolean),
        鍵: [...節.querySelectorAll(".about__k")].map((x) => x.textContent.trim()),
      };
    });
    must(r, "/privacy に、⚠ 何を数えているかの節が無い");
    // ⚠ **文で切ってから見る**（`CLAUDE.md` §9）。
    //   ⚠ **節の全文に対する test だと、⚠ どこかに似た語があれば通る**（⚠ 実際に踏んでいる）。
    const 言っている = (...語) => r.文.some((x) => 語.every((w) => x.includes(w)));
    must(r.鍵.includes("この端末に置く印"), `⚠ 端末の印の行が無い（${r.鍵.join(" / ")}）`);
    must(言っている("この端末の中に印"), "⚠ 印をどこに置くかを言っていない");
    must(言っている("Cookie ではない"), "⚠ Cookie ではないことを言っていない");
    must(言っている("日が変わると別の印"), "⚠ 印が 1 日で別のものになることを言っていない");
    must(言っている("座標", "町名", "送りません"), "⚠ 座標も町名も送らないと言っていない");
    must(言っている("スマホと PC", "結びつけることはしません"), "⚠ 端末をまたがないことを言っていない");
    // ⚠ **配信の仕組みの話と混ざらないこと。**⚠ **主語を限っていないと、⚠ 矛盾して読める**
    //   （⚠ すぐ下で「この端末の中に印を置きます」と言っているため）。
    must(!言っている("この端末の保存領域も", "この集計には使っていません"),
      "⚠ 配信の仕組みの話が、⚠ 主語を限らずに「使っていません」と言っている");
    return `${r.鍵.length} 行（${r.鍵.join(" / ")}）／ ${r.文.length} 文`;
  },
});
