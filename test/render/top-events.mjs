...
//
// ⚠ **`寄ると、地図として本当に近づく` は、⚠ 揺れていた 1 件**（hidetzu/konjaku#275）。
//   ⚠ **落ちたときに原因が読める形（層・`mapAwake`・枠の幅）も、⚠ 一緒に運んだ。**
//   ⚠ **2026-08-28 に「いまは再現しない」として閉じた。**⚠ **直したのではない。**
//     ⚠ **落ちた 2 回は手元。**⚠ **CI では一度も落ちていない**（⚠ 実測 2026-08-28）:
//       落ちた 2 日を含む窓（08-24〜08-26）  CI 105 回 — ⚠ すべて緑
//       診断を仕込んで以降（08-26〜08-28）   CI 129 回 — ⚠ このケースは全部緑
//       手元（2026-08-26）                  50 回で 0 回（⚠ 出力が 1 文字も同じ）
//   ⚠ **下の診断を消さない。**⚠ **これが罠**で、⚠ **次に落ちた 1 回で原因が確定する。**
//     ⚠ **消すと、⚠ また「12 件」しか残らない状態に戻る**（⚠ 過去 2 回がそれ）。
//   ⚠ **落ちたら reopen する。**⚠ **そのときの出力をそのまま貼る。**
//
