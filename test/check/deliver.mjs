...
// ⚠ **コメントを先に落とす**（`CLAUDE.md` §5）。⚠ **落とさないと、⚠ 上に書いた注記の字面を拾う。**
{
  ...
  // ⚠ **トリガ**（⚠ ここが `main` を含まないと、⚠ 出す段が一度も呼ばれない）
  //   ⚠ **2026-09-01 に `develop` を消した**（`docs/adr/0082`）。⚠ **枝は `main` の 1 本だけ。**
  const push = /^on:\n(?:.*\n)*?  push:\n((?:\s{4}.*\n)+)/m.exec(cleaned)?.[1] ?? "";
  /\bmain\b/.test(push)
    ? ok("push のトリガに main が居る（⚠ いまの主線。ここでしか全部は回らない）")
    : bad("push のトリガに main が居ない"
  ...
  if (!dep) {
    ...
    // ⚠ **出す枝は 1 つだけ**（2026-09-01。`docs/adr/0082`）。
    //   ⚠ **`||` で枝を足すと、⚠ どこから出たのかが追えなくなる。**
    //   ⚠ **大きな開発の枝を切っても、⚠ そこからは出さない。**
    /\|\||&&\s*github\.ref[\s\S]*github\.ref/.test(cond)
      ? bad(`出す段の枝が 1 つでない: ${JSON.stringify(cond)}`
          + "（⚠ **どこから本番が出たのかが追えなくなる**）")
      : ok("出す段の枝は 1 つだけ（⚠ どこから出たのかが追える）");
