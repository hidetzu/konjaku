...
// ⚠ **箱の内側の余白は、⚠ くり返されている 2 つだけ段にしてある**（hidetzu/konjaku#312）。
//
// ⚠ **色でやったのと同じ形**（hidetzu/konjaku#96）: ⚠ **段に在る値を、⚠ 規則へ直に書かない。**
//   ⚠ **これが無いと、⚠ 「段に無いから直に書く」が積み上がって元に戻る。**
//   ⚠ **`font-size` には既に同じ見張りがある**（⚠ 生の値を書いたら落とす）。
//
// ⚠ **段に無い `padding` は咎めない。**⚠ **未対応だからではない。**
//   ⚠ **2026-08-27 に Owner が決めた**: ⚠ **`padding` は段にしない**（`docs/adr/0041`）。
//   ⚠ **測った結果、⚠ 揃える根拠が無かった**（⚠ 実測 2026-08-27・`main` = `e217da9`）:
//       ⚠ 2 値の `padding` 39 件のうち、⚠ **縦（上下）は 13 種がほぼ平坦**（最頻値でも 6 件）。
//       ⚠ **どれに揃えても 33〜36 件が動き、⚠ 最大 7〜13px ずれる。**
//       ⚠ **縦の間（`--gap-*`）は最大 5px で収まった**のと、⚠ **性質が違う。**
//   ⚠ **`margin` は「2 つのものの間」で概念が 1 つ。**⚠ **`padding` は「この箱の内側」で、
//     ⚠ 箱ごとに正当な理由で違う。**⚠ **だから段にしない。**
//
// ⚠ **数は名乗る。**⚠ **ただし「まだ」とは言わない**（⚠ 未対応に読まれる）。
{
  const css = await readFile(join(PUB, "css", "tokens.css"), "utf8");
  ...
  for (const [f, s0] of [["index.html", src["index.html"]], ["peel.html", src["peel.html"]],
    ...
    // ⚠ **段に無い `padding` の数**（⚠ 落とさない。⚠ **決めた上でそうしている**）。
    //   ⚠ **`margin` は数えない。**⚠ **そちらは段から選ぶ決まり**で、⚠ 別の節が落としている。
    for (const m of body.matchAll(/padding[a-z-]*\s*:\s*([^;{}]*)/g))
      if (/\d+px/.test(m[1])) left++;
  }
  if (!steps.size) bad("tokens.css に箱の内側の余白の段が無い（⚠ この検査が何も見ていない）");
    ...
    ok(`箱の内側の余白は、くり返されている ${steps.size} つだけ段から借りている`
      + `（使用 ${hit.length} 箇所・直書き 0）。`
      + `⚠ **段に無い padding が ${left} 件ある。**⚠ **決めた上でそうしている**`
      + `（⚠ 揃える根拠が無かった。docs/adr/0041）`);
}
