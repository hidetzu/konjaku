...
if (!existsSync(NEXT)) bad("public/ が無い（⚠ 本番が配る先）");
  ...
  //   ⚠ **「ここに書いていないことは、していません」の姿勢は残す**（⚠ Owner 指示）。
  {
    ...
    // ⚠ **要約は 6 つ。**⚠ **Owner が挙げた 4 点 ＋ メール ＋ 計測**（2026-09-06）。
    ...
    if (見出し.length !== 6)
      欠け.push(`冒頭の要約が ${見出し.length} 点しかない（⚠ 6 点と決めてある）`);
    ...
    const 要る = {
      ...
      "メールを受け取ること":       /メール/,
      // ⚠ **計測すること**（2026-09-06。Owner 判断。`docs/adr/0102`）。
      //   ⚠ **書かないと「ここに書いていないことは、していません」が嘘になる。**
      //   ⚠ **同時に「位置情報は送信されません」も言う**（⚠ 何をしないかまでが 1 対）。
      "計測すること":               /記録します/,
      "位置を送らないこと":         /位置情報は送信されません/,
    };
