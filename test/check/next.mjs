...
if (!existsSync(NEXT)) bad("public-next/ が無い（ADR 0050 の器）");
  ...
  // ---- ⚠ ⑮ 広い幅で、⚠ サイトの案内へ行けなくなっていないか ----
  //
  // ⚠ **広い幅では、⚠ 名乗りのメニューを畳んで、⚠ ページの下が案内を引き受ける**
  //   （⚠ 同じ道を、⚠ 同じ画面に 2 つ出さないため）。
  // ⚠ **だから、⚠ ページの下が 1 つでも欠けると、⚠ 広い幅でそこへ行けなくなる。**
  //   ⚠ **狭い幅では行けるので、⚠ 字を見るだけの検査では気づけない。**
  //
  // ⚠ **地図の画面（`.brand--app`）は、⚠ ページの下を持てない**（⚠ 画面いっぱいが地図）。
  //   ⚠ **あちらは広い幅でもメニューが持ち続ける。**⚠ **持ち続けていることも見る。**
  {
    const 欠け = [];
    for (const f of readdirSync(NEXT).filter((x) => x.endsWith(".html"))) {
      ...
      const 地図の画面 = /<header class="brand[^"]*\bbrand--app\b/.test(src);
      if (地図の画面) {
        // ⚠ **ここはメニューが持ち続ける側。**⚠ **ページの下を持たない**ことを見る
        if (下.length) 欠け.push(`${f}: 地図の画面なのにページの下を持っている`);
        continue;
      }
      if (!下.length) { 欠け.push(`${f}: ページの下の案内が無い`); continue; }
      const 足りない = 案内.filter((h) => !下.includes(h));
      if (足りない.length) 欠け.push(`${f}: ページの下に ${足りない.join(" ")} が無い`);
    }
    欠け.length
      ? bad(`広い幅で、⚠ サイトの案内へ行けなくなる画面がある: ${欠け.join(" ／ ")}`
          + "。⚠ **広い幅ではメニューを畳むので、⚠ ページの下が同じ行き先を全部持つ**")
      : ok("v0.1.0 は、⚠ どの幅でもサイトの案内へ行ける"
          + "（⚠ 狭い幅は帯のメニュー ／ ⚠ 広い幅はページの下。⚠ 地図の画面はメニューが持ち続ける）");
  }
