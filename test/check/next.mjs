...
if (!existsSync(NEXT)) bad("public-next/ が無い（ADR 0050 の器）");
  ...
  //   ⚠ **「画面に見えている」は言えない**（⚠ 実描画が要る。⚠ v0.1.0 には無い）。
  //   ⚠ **見えているかは、⚠ tmp/attrib-check.mjs で測った**（⚠ 375/320px・明暗・写真中・一覧を開いた状態）。
  //   ⚠ **測っていないことを、⚠ 測ったと言わない**（掟 §1）。
  ...
  出典あり.length
    ? ok(`v0.1.0 の配信物に、地理院タイルの出典明示が書かれている（⚠ ${出典あり.length} ファイル。⚠ 見えているかは実描画では見ていない）`)
