...
const { places } = win.KonjakuPlaces;
...
{
  // 1検索1リクエストを守る。places() は応答を受け取るだけで通信しない
  const io = /\bfetch\s*\(|XMLHttpRequest|new\s+Image|import\s*\(/.test(src);
  io ? bad("places.js が通信している。1検索あたりの外部リクエストが増えている")
     : ok("1検索あたりの外部リクエスト: 1（並べ替えは応答の中だけで完結）");
}
