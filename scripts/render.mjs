...
const CASES = [
  ...
  //   依存を足すたびに人が思い出すのでは、いつか必ず落ちる。
  {
    ...
    async check(page, reqs) {
      ...
      // ⚠ 言い切りすぎない。地図を取りに行く相手には座標が渡る
      must(/こちらのサーバーには送りません/.test(foot),
        "何を送っていないのかが書かれていない");
      // ⚠ 文言そのものに縛らない（読点1つで落ちると、直すたびに検査を書き換えることになる）。
      //   見たいのは「提供元に座標が渡ると書いてあるか」。
      must(/提供元に[はも、]?.*座標が渡り/.test(foot),
        "提供元に座標が渡ることを書いていない（「どこにも送らない」は嘘になる）");
      ...
      //   残りは details に入れてよいが、**この2つは開かなくても読めること**。
