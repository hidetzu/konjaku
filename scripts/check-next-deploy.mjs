...
//
// ⚠ **`/version.json` は使えない。**⚠ あれは β 版の仕組みで、⚠ `public-next/` は持たない
...
//
// 使い方:
//   node scripts/check-next-deploy.mjs --origin=https://… --cb=<commit>
...
const DIR = join(ROOT, "public-next");
...
export const PREPENDED = new Map([
  ["robots.txt",
    ...
    + "⚠ 検索に出さないことは public-next/_headers の X-Robots-Tag が持つ"],
]);
...
async function main() {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    if (!ng.length) {
      console.log(`${targets.length} ファイルとも通った（${origin} は、この commit の public-next/ と同じ）`);
      return;
    }
...
if (process.argv[1] && process.argv[1].endsWith("check-next-deploy.mjs")) await main();
