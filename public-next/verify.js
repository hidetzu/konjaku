...
(function (global) {
  ...
  // ⚠ **第1層の文は public/words.js が持つ**（2026-08-20 に移した。hidetzu/konjaku#122）。
  //   ⚠ 以前はここに `plainPast()` があり、`もとは 水だった土地（旧水部）です。いまは …` を
  //     組んでいた。⚠ **同じ第1層を /peel は `この土地は 旧水部` と出しており、
  //     画面ごとに形が違った**（ADR 0030 §4 の実測）。
  //   ⚠ **平易な言い換え（`水だった土地`）は落とした。**ADR 0030 §4-5:
  //     **原典の語をそのまま出す。**⚠ 時間の語を使わずに済む。
  //   ⚠ **意味は、2026-08-22 から押さずに読める**（ADR 0031。hidetzu/konjaku#148）。
  //     ⚠ **区分名の下に 18 字の補助説明が 1 行**出る（字は words.js の GROUND_GLOSS）。
  //     ⚠ **ここが持つ文は変わっていない。**⚠ 補助説明は別の行で、⚠ **別の持ち主。**
  //   ⚠ **区分名は消していない。**国土地理院の区分名で、成因と災害リスクの記述がこれに紐づく。
  //   ⚠ **意味の言い換えはここでしない。**区分の意味は `landform.json` の `why` が持つ
  //     （こちらで言い換え始めると原典と食い違う）。
  ...
  function narrate(f) {
    ...
    if (l.ok) {
      // ⚠ **字は words.js の 1 か所。**⚠ トップ・/peel・共有カードが同じものを出す。
      //
      // ⚠ **2026-08-20 に、ここの言い回しを ADR 0030 §4 へ揃えた**（hidetzu/konjaku#122）。
      //   ⚠ **前は「もとは 水だった土地（旧水部）です。いまは 盛土地･埋立地 です。」**だった。
      //   ⚠ その形を選んだ 2026-08-17 の実測（初見 3 人）は残っているが、
      //     ⚠ **2026-08-19 に利用者役 4 名で測り直したら、3/4 が「もとは〜」を
      //     明治期（第2層）と取り違えた。**⚠ 新しいほうを採る。
      //   ⚠ **平易な言い換え（`水だった土地`）も落とした**（ADR 0030 §4-5）。
      //     ⚠ 意味は `landform.json` の `why` が持ち、根拠パネルと災害リスク欄で出している。
      //     ⚠ **2026-08-22 から、区分名の下にも 18 字の補助説明が 1 行出る**（ADR 0031）。
      //       ⚠ **原典の置き換えではない。**⚠ 根拠パネルの `why` は消していない。
      // ⚠ **人工地形が無い土地でも文が完成すること。**札幌・軽井沢で尻切れになった実績がある
      //   （2026-08-17）。⚠ `ground1Lines` は行で返すので、人工地形が無ければ 1 行で閉じる。
      out.push(KonjakuWords.ground1Text(l.value, l.artificial));
