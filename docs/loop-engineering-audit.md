...
| 何 | いま | 分類 |
|---|---|---|
| `.claude/skills/issue-work/SKILL.md` | Issue 取得 → きまりを読む → Owner Decisions 抽出 → **計画の承認** → 実装 → 検査 → 自己レビュー → コミット → PR → Completion Report | ⚠ **割らない**（D-1）。⚠ ただし §6 の検査の**書き方**は `verify` へ寄せた（2026-08-19） |
...
⚠ **いま困っていないなら割らない**（§4 D-1 で「割らない」と決着）。

⚠ **ただし §6（検査）だけは、書き方が `verify` と二重になっていた。**
⚠ **2 か所に書くと、片方だけ古くなる**（掟: 同じ問いに答える実装を2つ持たない）。
→ 2026-08-19 に `issue-work` §6 を `verify` への**参照**に置き換えた。
実測: `issue-work` の中の `npm run check` の記述は **7 → 0 件**（`verify` が 7 件）。
⚠ **`issue-work` に残したのは、作業としての義務 2 つだけ**
（Final Gate まで通す ／ 直した不具合を検査として残し、わざと壊す）。
...
## 4. 人が決めるもの（⚠ 2026-08-19 に Owner が回答済み）

⚠ **6 件とも答えが出ている。**⚠ **もう一度聞かない。**
⚠ 「いまは不要」であって「永久に不要」ではない。**必要になったのは、必要になってから。**

| # | 何 | 回答（2026-08-19） |
|---|---|---|
| D-1 | `issue-work` を割るか | ⚠ **割らない。**「Skill 数を増やすこと自体を目的にしない」。⚠ いま困っていない（10 段が長すぎて破綻した実例が無い） |
| D-2 | Issue Quality Gate を Skill で持つか、Issue テンプレートにも出すか | ⚠ **いまは不要。**Skill だけで持つ。⚠ テンプレートに寄せると、**外から報告する人の負担が上がる** |
...
⚠ **Loop Controller はまだ導入していません。**
⚠ **2026-08-19 に Owner が「いまは不要」と回答した**（§4 D-6）。
⚠ **手動ループを 1〜3 件試すのが先。**そこで出たことを見て、また判断する。
