...
| # | 題 | 一次分類 | ⚠ なぜ |
|---|---|---|---|
| #27 | 端末内だけで更新を知らせる**仕様を決める** | **NEEDS-HUMAN-DECISION** | ⚠ 題が「仕様を決める」。決めるのは人 |
| #24 | 出典ごとのデータ更新状況 | **REWRITE** | 方針は具体的だが、Acceptance Criteria が機械で判定できる形になっていない |
| #22 | 年代変更・判定完了が読み上げに伝わらない | ⚠ **KEEP に近い** | 現状の実測（`aria-live` 0 件）・対応方針・通知タイミングが具体的。⚠ **ただし文面は人が決める** |
| #21 | Performance Budget と回帰検査 | **SPLIT** | 「予算を決める」（人）と「検査を書く」（AI）が混在 |
| #15 | 表示基盤の共通化・文字拡大・テーマ | **SPLIT** | 3 つの独立した目的。⚠ トークン共通化／rem 化／テーマ切替 |
| #14 | 外部リンクを日本語 Wikipedia へ・三状態 | ⚠ **KEEP に近い** | Owner Decisions が本文にある。実測（2,220/2,367＝93.8%）もある |
| #12 | キーボード操作とフォーカス表示 | **SPLIT** | ⚠ 実測が 2 件以上ある（`/peel` の閉じたパネル・トップの検索欄）。別々に直せる |
| #11 | 「動きを減らす」で自前アニメを止める | ⚠ **KEEP に近い** | 実測（自前 0 件・transition 5＋5・animation 2）が具体的。⚠ 11 秒再生の扱いだけ判断が要る |
| #9 | 利用者向けの言葉に揃える | ⚠ **進行中**（この Issue で作業してきた） | ⚠ 進捗の出どころは `tmp/9/06-いまここ.md` |
...
|---|---|---|
| #11 | 自前コードに `prefers-reduced-motion` が **0 件** | ⚠ **変わっていない**（0 件） |
| #11 | トップ: `transition` 5 宣言・`animation` 2 宣言 | ⚠ **変わっていない**（5 / 2） |
| #11 | `/peel`: `transition` **5 宣言** | ⚠ **8 宣言に増えている**。さらに `scroll-behavior` が **2 件**（Issue に無い） |
| #11 | トップ: smooth scroll **6 か所** | ⚠ **7 か所** |
| #12 | `/peel` の閉じたパネルへ Tab が入る | ⚠ **`inert` が入っている**（`peel3d.js` の `sealOldControls`）。実描画「見えない操作に、キーボードで届かない」が見ている。⚠ **Issue が挙げた要素と同じかは、測り直しが要る** |
| #12 | トップの `#q` にフォーカス表示が無い | ⚠ **変わっていない**（`index.html:173` に `outline:none`）。⚠ さらに `#memo:focus` も `outline:none` |
| #12 | — | ⚠ **`focus-visible` は両画面とも 0 件。**つまり**前例が無い**ので、見せ方を決めるところから |
